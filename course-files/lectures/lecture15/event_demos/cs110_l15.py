...
def rectangle(top_left=(0, 0), width=25, height=50, color="hot pink", outline="", tag="", **kwargs):
    """
...
def oval(center=(0, 0), radius_x=25, radius_y=50, color="hot pink", outline="", tag="", **kwargs):
    """
...
def triangle(
    bottom_center=(0, 0), width=25, top_shift=0, height=0, color="hot pink", outline="", tag="", **kwargs
):
    """
...
def star(center=(0, 0), radius=50, color="hot pink", outer_radius=75, points=5, outline="", tag="", **kwargs):
    """
...
def listen_for(event, handler_function, override=False):
    '''
    Sets up a listener for a given event on our window.
    
    Args:
        event (`str`): The magic string that represents this event in the window
    ...
    if event not in event_translator and not override:
        raise(TypeError(f"The event you entered, {event}, isn't supported. Here are the supported events: {[i for i in event_translator]}"))
    
    event = event_translator[event]
