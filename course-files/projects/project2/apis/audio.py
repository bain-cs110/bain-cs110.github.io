...
try:
    ...
    LASTFM_KEY = secret_tokens.LASTFM_KEY
except:
    print("LAST.FM not available. See Prof Bain or post on edSTEM.")
...
__all__ = [
    ...
    "generate_albums_table",
    "generate_mixtape"
]


def get_genres(debug:bool = True):
    """
    Asks Spotify for a list of available genres. There are actually 6000+ valid
    genres on Spotify [all of which can be found here](https://everynoise.com/everynoise1d.html)
    but for our purposes it might be wise to limit yourself to just these.
...
def search_for_tracks(search_term: str, debug:bool = True, simplify: bool = True):
    '''
...
def _search_by_genres(genres: list, debug:bool = True, simplify=True):
    '''
    Retrieves a list of Spotify tracks, given some genre.

    Args:
        genres (`list`): A list of valid spotify genres
        debug (`bool`): Whether or not you want debug text to be printed.
        simplify (`bool`): Indicates whether you want to simplify the data that is returned.
    ...
    Returns:
        a `list` of tracks.
    '''
    url = f'https://api.spotify.com/v1/search?q=genre%3A{",".join(genres)}&type=track'
    data = _issue_get_request(url, debug=debug)
    if not simplify:
        return data
    return _simplify_tracks(data['tracks']['items'], debug=debug)
...
def _get_top_tracks_by_artist(artist_id: str, debug:bool = True, simplify: bool = True):
    """
    Only exists to allow for mock testing of underlying 
    """
    return get_top_tracks_by_artist(artist_id, debug, simplify)
...
def search_for_artists(search_term: str, debug:bool = True, simplify: bool = True):
    '''
    ...
    Returns:
        a `list` of artists.
    '''
    url = 'https://api.spotify.com/v1/search?q=' + search_term + '&type=artist&market=US'
    data = _issue_get_request(url, debug=debug)
    if not simplify:
        return data
...
def search_for_albums(search_term: str, debug:bool = True, simplify: bool = True):
    '''
    Retrieves a list of Spotify albums, given the search term passed in.

    Args:
        search_term (`str`): A search term (for an album), represented as a string.
        debug (`bool`): Whether or not you want debug text to be printed.
        simplify (`bool`): Indicates whether you want to simplify the data that is returned.
    ...
    Returns:
        * a `list` of albums.
    '''
    url = 'https://api.spotify.com/v1/search?q=' + search_term + '&type=album&market=US'
    data = _issue_get_request(url, debug=debug)
    if not simplify:
        return data
    return _simplify_albums(data['albums']['items'], debug=debug)


def search_for_playlists(search_term: str, debug:bool = True, simplify: bool = True):
    '''
...
def get_new_releases(debug:bool = True, simplify:bool = True):
    '''
    ...
    url = f'https://api.spotify.com/v1/search?q=tag:new&type=album&market=US&offset=0'
    data = _issue_get_request(url, debug=debug)
    if not simplify:
        return data
    return _simplify_albums(data['albums']['items'], debug=debug)
...
def get_albums_by_artist(artist_id:str, options:list = [], debug:bool = True, simplify:bool = True):
    '''
    Retrieves a list of Spotify albums by a given artist.

    Args:
        artist_id (`str`): A valid Spotify artist ID
        options (`list`): A list of special strings to indicate what sort of albums to look for. Valid
                          items are: `"album"`, `"single"`, "`compilation`", and `"appears_on"`.
        debug (`bool`): Whether or not you want debug text to be printed.
        simplify (`bool`): Indicates whether you want to simplify the data that is returned.
    ...
    _verify_artist_ids([artist_id])
    
    valid_options = ['album', 'single', 'compilation', 'appears_on']
    for option in options:
        if option not in valid_options:
            raise TypeError(f"The options input only accepts the following options: {valid_options}")
    
    if options:
        url = f'https://api.spotify.com/v1/artists/{artist_id}/albums?include_groups={",".join(options)}&market=US'
    else:
        url = f'https://api.spotify.com/v1/artists/{artist_id}/albums?market=US'
    data = _issue_get_request(url, debug=debug)
    if not simplify:
        return data
    return _simplify_albums(data['items'], debug=debug)

def _get_several_artists(artist_ids: list, debug:bool = True, simplify:bool = True):
    '''
    Retrieves a bunch of particular artists from Spotify API.

    Args:
        artist_id (`str`): The id of the Spotify artist.
        debug (`bool`): Whether or not you want debug text to be printed.

    Returns:
        a `dictionary` representing the artist
    '''
    if artist_ids:
        for i in artist_ids:
            if len(i) != 22:
...
def _get_several_tracks(track_ids: list, debug:bool = True, simplify: bool = True):
    '''
    Retrieves a bunch of particular tracks from Spotify API.
    ...
    if track_ids:
        ...
        raise TypeError("There are no Track ids in this list.")

    url = "https://api.spotify.com/v1/tracks?ids=" + ",".join(track_ids) + "&market=US"

    print(f"DEBUG - {'_get_several_tracks'}: Here's the request we're going to make.\n{url}\nYou can't access it in a browser, but you can double check the inputs you gave the function are part of the URL.")
...
def generate_mixtape(artist_ids: list = [], track_ids: list = [], genres: list = [], length:int = 20, debug: bool = True, practice:bool = True, simplify: bool = True):
    """
    Generate a mixtape based off the inputted artists, tracks, and genres. You must provide at least 1 input across artists, tracks, and genres.
    <mark>Keep in mind it does not accept artist names or track names.</mark> Specifying multiple genres might result in getting zero results.</mark>
    ...
    **Note**: In practice mode, this function will print a warning if inputs are invalid but will still return data.
    ...
    Args:
        ...
        length (`int`): How many tracks to return as part of the mixtape
        debug (`bool`): Whether or not you want debug text to be printed.
    ...
    Returns:
        * a `list` of tracks (dictionaries)
    """

    if not artist_ids and not track_ids and not genres:
        if practice:
            print(f"PRACTICE MODE: Note, no inputs were given.")
        else:
            raise Exception('Either artist_ids or track_ids or genres required')

    if practice:
        return _simplify_tracks(json.loads(TRACKS_JSON)['tracks'])

    _verify_artist_ids(artist_ids, practice=practice)
    _verify_track_ids(track_ids, practice=practice)

    track_list = []

    for artist in artist_ids:
        track_list += _get_top_tracks_by_artist(artist_id=artist, debug=debug, simplify=simplify)

    if track_ids:
        track_list += _get_several_tracks(track_ids, debug=debug, simplify=simplify)

    if genres:
        track_list += _search_by_genres(genres, debug=debug, simplify=simplify)

    track_list = _remove_duplicate_tracks(track_list)
    random.shuffle(track_list)

    return track_list[:length]


def _remove_duplicate_tracks(track_list):
    unique_track_ids = []
    unique_tracks = []
    for track in track_list:
        if track['id'] in unique_track_ids:
            continue
        else:
...
def _verify_track_ids(track_ids=[], practice = True):
    for i in track_ids:
        if len(i) != 22:
            if practice:
                print(f"PRACTICE MODE: Note, Track ID '{i}' is invalid.")
            else:
                raise TypeError(f"This input requires Track IDs but you gave it {i}.")


def _verify_artist_ids(artist_ids=[], practice = True):
    for i in artist_ids:
        if len(i) != 22:
            if practice:
                print(f"PRACTICE MODE: Note, Artist ID '{i}' is invalid.")
            else:
                raise TypeError(f"This input requires Artist IDs but you gave it {i}.")
...
def get_similar_tracks(artist_ids: list = [], track_ids: list = [], genres: list = [], debug: bool = True, practice:bool = True, simplify: bool = True):
    ...
    if artist_ids:
        ...
        if not practice:
            artists = _get_several_artists(artist_ids, debug=debug)
            print(artists)
            simple_artists = _simplify_artists(artists)

            random_artist = simple_artists[0]
    ...
    if track_ids:
        ...
        if not practice:
            tracks = _get_several_tracks(track_ids, debug=debug)
            simple_tracks = _simplify_tracks(tracks)

            random_track = simple_tracks[0]
    ...
    if practice:
        return _simplify_tracks(TRACKS_JSON)

    if random_artist and random_track:
        related_artists = _lastfm_get_similar_artists(random_artist['name'], debug=debug)
        related_artists_tracks = _lastfm_get_top_tracks(related_artists[0]['name'], debug=debug)

        pick_one = random.choice(related_artists_tracks)
        other_related_tracks = _lastfm_get_similar_tracks(pick_one['name'], debug=debug )

        related_tracks = _lastfm_get_similar_tracks(random_track['name'], random_track['artists'][0]['name'], debug=debug)

        return other_related_tracks + related_tracks

    elif random_track:
        related_tracks = _lastfm_get_similar_tracks(
            random_track["name"], random_track["artists"][0]["name"], debug=debug
        )
        return related_tracks

    elif random_artist:
        related_artists = _lastfm_get_similar_artists(random_artist['name'], limit=5, debug=debug)
        #print(related_artists)
        related_artists_tracks = _lastfm_get_top_tracks(related_artists[0]['name'], debug=debug)
        #print(related_artists_tracks)
        random_track = random.choice(related_artists_tracks)
        print(random_track)
        related_tracks = _lastfm_get_similar_tracks(
            random_track["name"],
            random_track["artist"]["name"],
            limit=10,
            debug=debug,
        )
        ...
        raise Exception(f"It looks like you either only provided a genre or we didn't get any data back from the API request. Here's what you gave: artist_ids: {artist_ids}\ntrack_ids: {track_ids}\ngenres: {genres}")


def _lastfm_get_similar_tracks(track_name:str, artist_name:str, limit=5, debug = False):

    if not track_name or not artist_name:
        raise Exception("Tried to get a track or artist with no name.")    

    url = f"http://ws.audioscrobbler.com/2.0/?method=track.getsimilar&artist={artist_name}&track={track_name}&api_key={LASTFM_KEY}&limit={limit}&format=json"

    if debug:
        print(
            f"DEBUG - {'_lastfm_get_similar_tracks'}\nHere's the request we're going to make.\n{url}\nYou can't access it in a browser, but you can double check the inputs you gave the function are part of the URL."
        )

    url = quote(url, safe="/:?&,=")

    cache_lookup = _check_cache_for(url)

    if not cache_lookup:    
        response = requests.get(url, verify=True)
        json_response = response.json()
        _save_to_cache(url, json_response)

    else:
        json_response = cache_lookup

    print(json_response)

    list_of_tracks = []
    for old_track in json_response['similartracks']['track']:
        spotify_formatted_track = {
            'id': old_track['mbid'],
            'name': old_track['name'],
            'url': old_track['url'],
            'image_url': [x['#text'] for x in old_track['image'] if x['#text'] == "large"],
            'artist': {
                'name': old_track['artist']['name'],
                'share_url': old_track['artist']['url'],
                'id': old_track['artist']['mbid'],
            }
        }
        list_of_tracks.append(spotify_formatted_track)

    return list_of_tracks


def _lastfm_get_top_tracks(name: str, limit=5, debug = False):
    if not name:
        raise Exception("Tried to get an artist with no name. Double check you're entering a valid artist.")
    
    url = f"http://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist={name}&api_key={LASTFM_KEY}&limit={limit}&format=json"
    
    if debug:
        print(f"DEBUG - {'lastfm_get_top_tracks'}\nHere's the request we're going to make.\n{url}\nYou can't access it in a browser, but you can double check the inputs you gave the function are part of the URL.")
    ...
    return json_response["toptracks"]['track']


def _lastfm_get_similar_artists(name: str, limit=5, debug = False):
    if not name:
        raise Exception("Tried to get an artist with no name. Double check you're entering a valid artist.")
    
    url = f"http://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist={name}&limit={limit}&api_key={LASTFM_KEY}&format=json"
    
    if debug:
        print(f"DEBUG - {'_lastfm_get_similar_artists'}\nHere's the request we're going to make.\n{url}\nYou can't access it in a browser, but you can double check the inputs you gave the function are part of the URL.")
    ...
    artists = json_response.get('similarartists').get('artist')
...
def _check_cache_for(url:str, debug=False)   :
    # Check cache
    if os.path.isfile(SPOTIFY_CACHE):
        with open(SPOTIFY_CACHE, 'rb') as file:
            cache = pickle.load(file)
    ...
    if url in cache:
        if debug:
            print(f"DEBUG - {'_check_cache_for'}: Found previous request! Returning cached result.")
        return cache[url]
    else:
        return None
...
def generate_tracks_table(tracks: list, to_html: bool = False):
    ...
    if to_html:
        return _get_tracklist_table_html(tracks)
...
def generate_albums_table(albums: list, to_html: bool = False):
    """
    Function that builds a string representation of a list of albums (dicitonaries).

    Args:
        albums (`list`): list of albums (dictionaries).
        to_html (`bool`): If `True` it will generate an HTML version (for an email or web page) and if `False` (default) will generate a string to print in Python.

    Returns:
        * a `str` that has a table in it
    """

    if to_html:
        return _get_album_table_html(albums)

    line_width = 95
    text = ""
    template = "{0:2} | {1:<30.30} | {2:<30.30} | {3:<22.22}\n"

    # header section:
    text += "-" * line_width + "\n"
    text += template.format("", "Name", "Primary Artist", "Release Date")
    text += "-" * line_width + "\n"
...
def _get_album_table_html(albums: list):
    template = """
        <tr>
            <td {css}>{name}</td>
            <td {css}><img src="{image_url}" /></td>
            <td {css}>{primary_artist}</td>
    ...
    for album in albums:
        rows.append(
            template.format(
                css=cell_css,
                name=album.get("name"),
                image_url=album.get("image_url_small"),
                primary_artist=album.get("artist").get('name'),
...
def _get_tracklist_table_html(tracks: list):
    '''
    ...
    if os.path.isfile(SPOTIFY_JSON):
        with open(SPOTIFY_JSON) as json_file:
            ...
            if time.time() >= float(data['expires']):
                if debug:
                    print(f"DEBUG - {'_generate_authentication_header'}:\n Auth token expired, time to refresh!")
        ...
        if not backup:
            try:
                ...
                with open(SPOTIFY_JSON, "w") as outfile:
                    json_object = json.dumps({ 'token': token, 'expires': time.time() + 3540}, indent=4)
                    outfile.write(json_object)
                
            except Exception as e:
                if debug:
                    print(f"DEBUG - {'_generate_authentication_header'}:\nCouldn't use either default API Key. Trying backup!")
                raise Exception(e)
...
def _issue_get_request(url, debug = True, practice = True):
    ...
    * practice (bool): Whether or not to return real data or practice data
    ...
    '''
    if debug:
        print(f"DEBUG - {'_issue_get_request'}:\nHere's the request we're going to make.\n", url, "\nYou can't access it in a browser, but you can double check the inputs you gave the function are part of the URL.")
    ...
    if url in cache:
        if debug:
            print(f"DEBUG - {'_issue_get_request'}:\nFound previous request! Returning cached result.")
        return cache[url]
    ...
    if response.status_code == 429:
        ...
        if debug:
            print(f"DEBUG - {'_issue_get_request'}:\nWe're going to try to use the backup...")
...
def _simplify_albums(albums: list, debug=True):
    '''
    Private function. Simplifies the Spotify artist data so that each dictionary only contains
    a few key features (the original dictionary is quite large).
    ...
    * artists (list): The original artists data structure returned from Spotify.
    ...
    for item in albums:
        album = {
            ...
            'share_url': 'https://open.spotify.com/album/' + item['id']
        }
        try:
            album['image_url'] = item['images'][0]['url']
            album['image_url_small'] = item['images'][-1]['url']
        except Exception:
            pass
        simplified.append(album)
    return simplified
    ...
    * playlists (list): The original playlist data structure returned from Spotify.
    ...
    try:
        ...
        for item in playlists:
            # Winter 25 - some playlists come back as None
            if item is None:
                continue
            simplified.append({
                'id': item['id'],
        ...
        raise Exception(
            f"The following playlist data structure could not be flattened:\n{playlists}"
        )
