...
class TestYelp(unittest.TestCase):
    ...
    def test_execute_business_queries_just_one_simplified(self):
        ...
        # Note sure why this check is here...maybe only sometimes gives back price?
        if 'price' in keys:
            keys.remove('price')
        self.assertSetEqual(keys, set([
             'transactions', 'image_url', 'categories', 'display_address', 'id', 'rating',
             'review_count', 'share_url', 'coordinates', 'display_phone', 'name'
        ]))
