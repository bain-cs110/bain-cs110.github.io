...
__all__ = [
   "make_grid", "draw_letter_in_grid", "game_over", "color_a_grid_square"
]
...
def draw_letter_in_grid(canvas, letter, grid_coord, past_guess=False):
    '''
...
def game_over(canvas, happy = False):
    '''
    Ends the game.
    ...
    Returns:
      * `None`. (Draws to the `canvas` using popup; has side-effects)
    '''
    if happy:
...
def color_a_grid_square(canvas, color, grid_coord):
    '''
