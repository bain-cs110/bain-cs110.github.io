...
canvas = Canvas(gui, 
                ...
                background='white')
hw7_utilities.setup_window(canvas)
...
def finalize_guess(guess, hint):
    ...
    if hint == "🟩🟩🟩🟩🟩":
        hw7_utilities.game_over(happy=True)
    elif len(game_data['past_guesses']) == 6:
        hw7_utilities.game_over()
...
def show_past_guess(past_guess, guess_number, hint):
    ...
    for i in range(0, 5):
        ...
        if hint[i] == "🟩":
            # TODO: replace pass with a call to `hw7_utilities.color_a_grid_square` to color the
        ...
        # they should be marked as finalized) by calling `hw7_utilities.draw_letter_in_grid`.
...
def show_game_board():
    ...
    '''
    # Clear the screen.
    hw7_utilities.delete("all")

    # Draw the grid
    hw7_utilities.make_grid(SCREEN_WIDTH, SCREEN_HEIGHT)
    ...
    for i in range(0, 5):
        if i >= len(current_guess):
            break
        hw7_utilities.color_a_grid_square(DEFAULT_COLOR, (i, guess_count))
        hw7_utilities.draw_letter_in_grid(current_guess[i], (i, guess_count))
...
hw7_utilities.setup_listener('<Key>', handle_typing)
