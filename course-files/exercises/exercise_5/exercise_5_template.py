from cs110_ex5 import *
_ignore = setup_shapes("Exercise 5", background="light blue", grid=True, width=800, height=800)
ticks = 0 
...
def go():
    ...
    move_by("test-square", x_shift=25, y_shift=0)

    if get_left("test-square") >= 400:
        # when the left side of the test-square is 400, celebrate!
        print("Hooray!", get_left("test-square"))
        

########################## YOUR CODE ABOVE THIS LINE #######&&!*%^#######################
## DO NOT MODIFY ANYTHING BELOW (otherwise we won't be able to grade your file)
clear_window() ## Nothing should be drawn outside of SETUP and GO
setup()
while True:
    go()
    _ignore.update()
