...
def move_by(shape, x_shift=0, y_shift=0):
    """
...
def make_grid(c, w, h, interval=100):
    ...
    for i in range(0, w, interval):
        _a_canvas.create_line(i, 0, i, h, tag="grid", fill="black")
    # Creates all horizontal lines at intervals of 100
    for i in range(0, h, interval):
        _a_canvas.create_line(0, i, w, i, tag="grid", fill="black")
    # Creates axis labels
    offset = 2
    for y in range(0, h, interval):
        for x in range(0, w, interval):
            _a_canvas.create_oval(
                x - offset, y - offset, x + offset, y + offset, fill="black", tag="grid"
            )
