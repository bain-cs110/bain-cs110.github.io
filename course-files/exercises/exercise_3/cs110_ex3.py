...
def _safe_color(color: str):
    ...
    if not isinstance(color, str):
        raise TypeError(f"Colors must be a string! You entered: {color}")
...
def rectangle(
    top_left=(0, 0), width=25, height=50, color="hotpink", outline="", tag="", **kwargs
):
    """
...
def wedge(
    center=(0, 0), radius=25, angle=180, color="hotpink", outline="", tag="", **kwargs
):
    """
    A reporter function that draws a circle.
    Args:
...
def diamond(
    center=(0, 0), width=25, height=50, color="hotpink", outline="", tag="", **kwargs
):
    """
    A reporter function that draws a rectangle.
    Args:
