...
If you have your Python installation setup as per [Lecture 17](../lectures/week07-lecture02) you'll need to use `pip` to install three packages: `sendgrid`, `requests`, and then to upgrade `certifi`. If `pip3` doesn't work on your computer (and you made sure to follow the directions in Lecture 17), then try replacing `pip3` with `pip`. (If you're on Windows, you might also try `py -m pip install requests` and the same syntax to install the other libraries).
...
**Note:** Even if your tests above pass, it's possible that when you use the Twilio library on a Mac you may encounter a security (SSL) error. To fix this, please open your finder and navigate to your `Applications > python3.x` folder (or whichever python version you have installed). Then, double-click on the `Install Certificates.command` file.
...
1. Retrieve the restaurants, and allow the user to specify an additional keyword before executing the search.
...
#### [3 Points] Extra Credit: Add a player to the email you send
Generate some Player HTML (either for a track, artist, or playlist) and include it in the email you send to the user.
