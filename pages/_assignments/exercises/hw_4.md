...
In this homework assignment, you'll be moving from making "static" pictures like in HW2 and HW3, to making live animations! Make sure to complete [Tutorial 3](tutorial03) before attempting this homework. Once you've finished Tutorial 4, you'll see how the `utilities.update_position` function makes our work a little easier by getting rid of some of the steps of animation.
...
<a class="nu-button" href="/course-files/homeworks/utilities.py" target="_blank">
...
In the `utilities.py` file you've downloaded. There's a `make_cloud` function that accepts the following inputs:
1. a `canvas` to draw on 
2. a `center` tuple `(x,y)` to draw the center of the cloud.

In `hw04.py`, replace the code on lines 12-15 (which is repetitive) with a loop (any kind of loop you want) that makes at least 10 to 30 clouds the top portion of the screen. Hints:
...
* Otherwise, if you shape is drawn by calling a helper function (e.g `make_oval` or `make_square`) then you should follow these steps:
...
**Last but not least**, you need to actually add the parameter to your function call in the original `make_creature` body. Here's an example of doing this with my square face dude from <a href="https://campuswire.com/c/GB09BA912/feed/148" target="_blank">Campuswire post</a>.
...
def make_creature(a_canvas, center, width):
    ...
    make_square(a_canvas, (center_x - width / 3, center_y + width / 6), big_square_width / 6, color="purple")  # left eye
    make_square(a_canvas, (center_x + width / 6, center_y + width / 6), big_square_width / 6, color="white")  # right eye
    make_square(a_canvas, (center_x - width / 60, center_y - width / 60), width / 30, color="white")  # nose
```
...
def make_creature(a_canvas, center, width, my_tag=""):
    ...
    center_x = center[0]
    center_y = center[1]
    make_square(a_canvas, (center_x - width / 2, center_y - width / 2), width, color="green", tag=my_tag) # face
    make_square(a_canvas, (center_x - width / 3, center_y + width / 6), big_square_width / 6, color="purple")  # right eye
    make_square(a_canvas, (center_x + width / 6, center_y + width / 6), big_square_width / 6, color="white")  # face eye
    make_square(a_canvas, (center_x - width / 60, center_y - width / 60), width / 30, color="white")  # nose
```
...
def make_creature(a_canvas, center, width, my_tag=""):
    ...
    center_y = center[1]
    make_square(a_canvas, (center_x - width / 2, center_y - width / 2), width, color="green", tag=my_tag) # face
    make_square(a_canvas, (center_x - width / 3, center_y + width / 6), big_square_width / 6, color="purple", tag=my_tag)  # right eye
    make_square(a_canvas, (center_x + width / 6, center_y + width / 6), big_square_width / 6, color="white")  # face eye
    make_square(a_canvas, (center_x - width / 60, center_y - width / 60), width / 30, color="white")  # nose
```
...
def make_creature(a_canvas, center, width, my_tag=""):
    ...
    make_square(a_canvas, (center_x - width / 2, center_y - width / 2), width, color="green", tag=my_tag) # face
    make_square(a_canvas, (center_x - width / 3, center_y + width / 6), big_square_width / 6, color="purple", tag=my_tag)  # right eye
    make_square(a_canvas, (center_x + width / 6, center_y + width / 6), big_square_width / 6, color="white", tag=my_tag)  # face eye
