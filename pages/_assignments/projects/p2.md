...
Download the [my_token.py](https://canvas.northwestern.edu/files/15968762/download?download_frd=1) file from Canvas and save it inside of the `apis` folder (included in the project starter code). If this link doesn't work, you might not be logged into Canvas. You can either go to Canvas, our course, and then the files tab to download directly or just make sure you're logged into Canvas and the link should start working.
### C. Run the verification script
Inside the `tests` folder, open the `run_verification.py` file and run it. While it's running it will ask you for an email address to test that part of your setup.
