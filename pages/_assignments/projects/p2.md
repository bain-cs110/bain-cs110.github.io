...
1. Other logistics:
    * *You are welcome to collaborate in groups of 2-4*.     
    * <mark>You must signup for a "Team" on Canvas if you're working in a group. Go to the People tab of our Canvas page, click on the P2 Teams, and with your group, sign up for any one of the 200 available groups.</mark> Note: that these sign-ups will close on Wednesday Fe 31st.
...
In order to complete P2, you'll need to install three external Python packages and upgrade an existing one. If you'd like to try to use the command prompt, [visit this `pip` page]({{site.url}}/resources/pip). Otherwise, inside the starter files, open the `install_python_packages.py` file and then run it. You'll be presented with 6 options, each with a number associated with them.
