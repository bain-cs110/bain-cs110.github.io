...
> **Note**: PLEASE read through the instructions in full before starting. It'll only take 5 minutes and it'll make things much more clear. <mark>Code templates won't be available until Friday evening.</mark> So you might as well read in the instructions in the mean time!
...
Download the `secret_tokens.py` file from the Canvas files tab and save it inside of the `apis` folder (included in the project starter code). **Make sure to move it into the `apis` folder!**. (Not available until after late on Friday)
...
<mark>Please read through the assignment text BEFORE downloading the template files (not available until later on Friday).</mark>
<br>
