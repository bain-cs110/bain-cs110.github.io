...
Otherwise, inside the starter files, open the `install_python_packages.py` file and then run it. You'll be presented with 6 options, each with a number associated with them. 

1. To install `sendgrid` (which is made by a copmany called Twilio) and `requests`, you'll first enter option `0`, then when the program asks which package you want to install, you'll enter the name of the package you want to install. It'll install it and then go back to the menu. 
2. Then pick `0` again and install the second module.
...
<mark>IF YOU DOWNLOADED THE SECRET TOKENS FILE PRIOR TO 1PM ON FRIDAY MAY 26 YOU'LL NEED TO REDOWNLOAD IT AND REPLACE THE OLDER VERSION!</mark>

Download the [secret_tokens.py](https://canvas.northwestern.edu/files/16702153/download?download_frd=1) file from the Canvas files tab and save it inside of the `apis` folder (included in the project starter code). If this link doesn't work, just go to our Canvas page, click on the Files tab, and download the file called `secret_tokens.py` (click the 3 dots next to the file and select download). **Make sure to move it into the `apis` folder!**
...
If you get a `SSL: CERTIFICATE_VERIFY_FAILED` error, we can fix that! **This is the most common error and often effects Macs** when you run the above (or when trying to send an email), and has to do with a technology that helps you securely connect to web pages called SSL. To fix this, please open your finder and navigate to your `Applications > Python 3.11` folder (or whichever python version you have installed). Then, double-click on the `Install Certificates.command` file. Your email functions should now work!
...
You will give the user the option tos specify an additional search term for their query (e.g. `hamburgers`) to further narrow in on particular businesses.
