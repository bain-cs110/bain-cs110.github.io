...
In order to complete P2, you'll need to install three external Python packages and upgrade an existing one. 

<details>
<summary>Want to try installing via <code>pip</code>? [Advanced]</summary>
If you'd like to try to use the command prompt, <a href="{{site.url}}/resources/pip">visit this page.</a>
</details>
...
<ol>
<li>To install <code>sendgrid</code> (which is made by a compnay called Twilio), <code>requests</code>, and <code>customtkinter</code>, you'll first enter option <code>0</code>, then when the program asks which package you want to install, you'll enter the name of the package you want to install. It'll install it and then go back to the menu.</li>
...
Inside the `tests` folder, open the `run_verification.py` file and run it (<mark>IF YOU GET AN ERROR COME BACK TO THIS PAGE AND KEEP READING</mark>). If you get a message that looks like this (after entering <b>your</b> email addresss) then you're good to go!
