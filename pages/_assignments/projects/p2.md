...
1. Get oriented with the relevant helper modules <a href="{{site.baseurl}}/course-files/projects/project02/docs/apis/yelp.html" target="_blank">apis.yelp</a>, <a href="{{site.baseurl}}/course-files/projects/project02/docs/apis/spotify.html" target="_blank">apis.spotify</a>, <a href="{{site.baseurl}}/course-files/projects/project02/docs/apis/twilio.html" target="_blank">apis.twilio</a>, which will help you interact with the service providers (Yelp, Spotify, and Twilio)
...
1. Other logistics:
    * *You are welcome to collaborate in groups of 2 or 3*. You are not allowed to work in groups of larger than 3.
    * <mark>You must signup for a "Team" on Canvas if you're working in a group. Go to the People tab of our Canvas page, click on the P2 Teams, and with your group, sign up for any one of the 200 available groups.</mark> Note: that these sign-ups will close on Wednesday May 31st.
...
In order to complete P2, you'll need to install three external Python packages and upgrade an existing one. If you'd like to try to use the command prompt, [visit this `pip` page](/resources/pip). Otherwise, inside the starter files, open the `install_python_packages.py` file and then run it. You'll be presented with 6 options, each with a number associated with them.

1. To install `sendgrid` (which is made by a copmany called Twilio) `requests`, and `customtkinter`, you'll first enter option `0`, then when the program asks which package you want to install, you'll enter the name of the package you want to install. It'll install it and then go back to the menu.
2. Then pick `0` again and install the second and third modules.
...
Inside the `tests` folder, open the `run_verification.py` file and run it (<mark>IF YOU GET AN ERROR COME BACK TO THIS PAGE AND KEEP READING</mark>). If you get a message that looks like this (after entering your email addresss) then you're good to go!
...
Additionally, you'll need to know how to "print" information to the GUI textbox and how to ask the user to input information into the GUI. For these two actions, check out the documentation linked below, for the `gui.show_text` and `gui.popup_input` functions.

* * *

## Option 1: Yelp

<a class="nu-button" href="{{site.baseurl}}/course-files/projects/project2/docs/apis/yelp.html" target="_blank">
    Yelp Docs <i class="fas fa-download"></i>
</a>

For option 1, you are going to use the Yelp API to create a restaurant recommendation system. I have created a `yelp` module for you, located in the apis folder to help you. All of the `apis/yelp.py` functions <a href="{{site.baseurl}}/course-files/projects/project02/docs/apis/yelp.html" target="_blank">have been documented here</a>. Here is a video of how your program might work:

<iframe src="https://northwestern.hosted.panopto.com/Panopto/Pages/Embed.aspx?id=fb642536-eb40-473b-8f37-b01801858cab&autoplay=false&offerviewer=true&showtitle=true&showbrand=true&captions=true&interactivity=all" height="405" width="720" style="border: 1px solid #464646;" allowfullscreen allow="autoplay"></iframe>
...
After the user has previewed a restaurant, ask them if they want to email the restaurant recommendation so someone. If so, email them a link to the restaurant and some details about it (including some reviews). Use the `twilio.send_mail` function to help you.
...
<a class="nu-button" href="{{site.baseurl}}/course-files/projects/project02/docs/apis/spotify.html" target="_blank">
    Spotify Docs <i class="fas fa-download"></i>
</a>

For option 2, you are going to use the Spotify API to create a music recommendation system. I have created a spotify module for you, located in the apis folder to help you. All of the `apis/spotify.py` functions <a href="{{site.baseurl}}/course-files/projects/project02/docs/apis/spotify.html" target="_blank">have been documented here</a>. Here is a demo video of how your program should work:

<iframe src="https://northwestern.hosted.panopto.com/Panopto/Pages/Embed.aspx?id=b7ed1f53-4a31-4874-a513-b0180188ee7e&autoplay=false&offerviewer=true&showtitle=true&showbrand=true&captions=true&interactivity=all" height="405" width="720" style="border: 1px solid #464646;" allowfullscreen allow="autoplay"></iframe>
...
3. Ask the user if they want to email the tracks to someone, and if so, send an email of the list of track recommendations. Use the `spotify.generate_tracks_table` function to help you build a nice tracklist (see the video) that can be emailed.
...
<mark>For offical rules reasons, the project is due at the end of our assigned Final Exam day which is Tuesday, June 6th at 11:59pm CST. If you turn it in by this deadline, you'll receive 3 extra points (still capped at 6 total extra points). The last moment to turn in Project 2 will be 48 hours later at 11:59pm CST on Thursday June 8th.</mark> Remember Canvas maintains its own clock...don't risk a last second submission.
...
1. Upload an MP4/MKV/MOV (NOT a link but an actual file) that demos your system to the VIDEO assignment on Canvas. You can see [Project 1](p1) for more details on how to do that.
...
<a class="nu-button" href="{{site.baseurl}}/course-files/projects/project02_template.zip" target="_blank">
...
<a class="nu-button" href="{{site.baseurl}}/course-files/projects/project02/docs/apis/yelp.html" target="_blank">
    Yelp Docs <i class="fas fa-download"></i>
</a>
...
<a class="nu-button" href="{{site.baseurl}}/course-files/projects/project02/docs/apis/twilio.html" target="_blank">
